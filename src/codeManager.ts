...
export class CodeManager implements vscode.Disposable {
    ...
    public runByLanguage(): void {
        ...
        const config = this.getConfiguration("jrx");
    ...
    private stopRunning() {
        if (this._isRunning) {
            ...
            vscode.commands.executeCommand("setContext", "jrx.codeRunning", false);
    ...
    private initialize(): void {
        this._config = this.getConfiguration("jrx");
    ...
    private async executeCommandInOutputChannel(executor: string, appendFile: boolean = true) {
        ...
        vscode.commands.executeCommand("setContext", "jrx.codeRunning", true);
